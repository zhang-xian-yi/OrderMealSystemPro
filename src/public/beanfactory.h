...
{
public:
    virtual AbstractModel_t* createBean() = 0;
protected:
...
public:
    ...
    bool registrarBean(IRegistrar<AbstractBean_t>* registrar,const int& id)
    {
        m_Registry[id] = registrar;
        return true;
    }
    ...
    AbstractBean_t* createBean(const int& id)
    {
        ...
        {
            return m_Registry[id]->createBean();
        }
        else
        {
            /*未找到*/
...
private:
   ...
   std::map<int, IRegistrar<AbstractBean_t> *> m_Registry;
};
...
public:
    /*将产品直接注册到单例模板工厂中*/
    explicit BeanRegistrar(const int& id)
    {
